...
import { ResultObject } from '@/api/baseApiResponseModel/baseApiResponseModel';
import { ReportAccountDetailRequestModel, ReportAccountListRequestModel, ReportAccountListResponseModel } from '@/api/features/report/model/ReportModel'; // Cập nhật đường dẫn
import { IReportRepo } from '@/api/features/report/ReportRepo'; // Cập nhật đường dẫn
import dayjs from 'dayjs';
import { useEffect, useState } from 'react';
...
const ReportAccountViewModel = (repo: IReportRepo) => {
  ...
  const getDetail = async (params: ReportAccountDetailRequestModel) => {
    try {
      setDetailLoading(true);
      const res = await repo.getAccountDetail(params);
      if (res?.data) {
        setDetail(res?.data);
      }
    } catch (error) {
  ...
  const deleteReport = async (params: ReportAccountDetailRequestModel) => {
    try {
      setDeleteLoading(true);
      const res = await repo.deleteAccountReport(params);
      if (res?.message === 'Success') {
        setResultObject({
          type: 'success',
          message: "Đã từ chối thành công!",
  ...
  const acceptReport = async (params: ReportAccountDetailRequestModel) => {
    try {
      setAcceptLoading(true);
      const res = await repo.acceptAccountReport(params);
      if (res?.message === 'Success') {
        setResultObject({
          type: 'success',
          message: "Đã chấp nhận thành công!",
  ...
  const activateReport = async (params: ReportAccountDetailRequestModel) => {
    try {
      setActiveLoading(true);
      const res = await repo.activateAccountReport(params);
      if (res?.message === 'Success') {
        setResultObject({
          type: 'success',
          message: "Đã kích hoạt lại tài khoản!",
