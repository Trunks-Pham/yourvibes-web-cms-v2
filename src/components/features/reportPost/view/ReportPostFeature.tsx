...
const ReportPostFeature = () => {
  ...
  const {
    ...
    acceptReport,
    activateReport,
  } = ReportPostViewModel(defaultReportRepo); // Sử dụng defaultReportRepo
    ...
    <CardFeature title="Báo cáo bài viết">
      <>
        ...
        <Table
          ...
          dataSource={reportedList}
          rowKey={(record) => `${record.user_id}-${record.reported_post_id}`} // Cập nhật rowKey
          pagination={{
            showSizeChanger: true,
            pageSizeOptions: [10, 20, 50, 100],
